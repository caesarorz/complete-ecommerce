...
class UserManager(BaseUserManager):
    def create_user(self, email, full_name=None, phone_number=None, password=None, is_active=True, is_staff=False, is_admin=False):
        if not email:
            raise ValueError(u"Users must have an email address")
            raise ValueError(
                u"Los usuarios deben tener una direccion de correo")
        if not password:
            raise ValueError(u"Los usuarios deben tener una contrasena")
        user_obj = self.model(
...
# for future use, extend the custom user to add new features.

# such as location, full and last name, type of customer (on eto five starts)
...
class EmailActivation(models.Model):
    ...
    #             subject = "Verificación de correo de un click"
    ...
    #                             recipient_list,
    #                             html_message=html_,
    #                             fail_silently=False,  # email did not go throgh, so while testing keep it quiet
