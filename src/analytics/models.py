...
class ObjectViewedQuerySet(models.query.QuerySet):
    def by_model(self, model_class, model_queryset=False):
        c_type = ContentType.objects.get_for_model(model_class) # content type
        return self.filter(content_type=c_type)
        # qs = self.filter(content_type=c_type)
        # if model_queryset:
        #     viewed_ids = [x.object_id for x in qs]
        #     return model_class.objects.filter(pk__id=viewed_ids)
        # return qs
...
def object_viewed_receiver(sender, instance, request, *args, **kwargs):
    c_type = ContentType.objects.get_for_model(sender) # content type
    user = None
