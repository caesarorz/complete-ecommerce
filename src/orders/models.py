...
class Order(models.Model):
    ...
    payment_method      = models.CharField(max_length=120, null=True, blank=True, choices=ORDER_PAYMENT_METHOD)
    cash_amount         = models.DecimalField(max_digits=100, decimal_places=2, null=True, blank=True)
    cash_change         = models.DecimalField(max_digits=100, decimal_places=2, null=True, blank=True)
    shipping_total      = models.DecimalField(default=500, max_digits=100, decimal_places=2)
    ...
    def get_status(self):
        if self.status == 'paid' or self.status == 'created':
    ...
    def check_done(self):
        ...
        if billing_profile and shipping_done and total > 0: #         if billing_profile and shipping_done and billing_address and total > 0:
            return True
        return False
...
def post_save_order(sender, instance, created, *args, **kwargs):
    print("Running... and post_save order")
    if created:
...
class ProductPurchaseManager(models.Manager):
    ...
    def products_by_request(self, request):
        ...
        variation_qs = Variation.objects.filter(id__in=ids_).distinct()
        return variation_qs
...
class ProductPurchase(models.Model):
    ...
    product             = models.ForeignKey(Variation, on_delete=models.CASCADE) # product.productpurchase_set.count()
