...
class BillingProfileManager(models.Manager):
    def new_or_get(self, request):
        ...
        if user.is_authenticated():
            """logged user checkout, remember payment"""
            obj, created = self.model.objects.get_or_create(
                                                user=user,
                                                email=user.email)
        elif guest_email_id is not None:
            """guest user checkout, auto reloads payments"""
...
class BillingProfile(models.Model):
    ...
    #Custom id in Stripe or Braintree
...
<Card card id=card_1C7PVrDNpBZr9653rGIcbKRM at 0x00000a> JSON: {
  "id": "card_1C7PVrDNpBZr9653rGIcbKRM",
  "object": "card",
  "address_city": null,
...
class ChargeManager(models.Manager):
    def do(self, billing_profile, order_obj, card=None):
        ...
        c = stripe.Charge.create(
          amount = int(order_obj.total * 100), # 39.19 * 100 -> 3919
          currency = "usd",
